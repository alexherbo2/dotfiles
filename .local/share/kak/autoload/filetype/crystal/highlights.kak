...
add-highlighter shared/crystal/code/ regex '\b[A-Z]\w*\b|@@?\w+\b' 0:variable
...
add-highlighter shared/crystal/line_comment region '#(?!\{)' '$' group
add-highlighter shared/crystal/line_comment/ fill comment
add-highlighter shared/crystal/line_comment/ regex '\B`[#.]?\w+[?!]?`\B|\B(?<!\*)\*\w+[?!]?\*(?!\*)\B|\b(?:BUG|DEPRECATED|EXPERIMENTAL|FIXME|NOTE|OPTIMIZE|TODO|WARNING)\b|\B:(?:ditto|nodoc|inherit):$' 0:meta
...
add-highlighter shared/crystal.string_with_escape_sequences_and_interpolation/content/ regex '\\#|\\[\\abefnrtv]|\\[0-7]{1,3}|\\(x[0-9a-fA-F]{2}|u[0-9a-fA-F]{4}|u\{[0-9a-fA-F]+\})' 0:value
...
add-highlighter shared/crystal/single_quoted_string/ fill string
...
add-highlighter shared/crystal/double_quoted_string/content/ regex '\\["#]|\\[\\abefnrtv]|\\[0-7]{1,3}|\\(x[0-9a-fA-F]{2}|u[0-9a-fA-F]{4}|u\{[0-9a-fA-F]+\})' 0:value
